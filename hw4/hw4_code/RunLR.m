...
function RunLR()
  ...
  for kInd = 1:length(kVals)
    ...
    for rep = 1:100
      ...
      % Test classifier on training set
      [yHatTrain,numErrorsTrain] = LR_PredictLabels(XTrainSubset,yTrainSubset,wHat);
      trainingError(kInd) = trainingError(kInd)+numErrorsTrain/k;
      % Test classifier on test set
      [yHatTest,numErrorsTest] = LR_PredictLabels(XTest,yTest,wHat);
      testError(kInd) = testError(kInd)+numErrorsTest/m;
    end
    trainingError(kInd) = trainingError(kInd)/100;
    testError(kInd) = testError(kInd)/100;
  endfor
